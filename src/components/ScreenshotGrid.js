export default function ScreenshotGrid({ items = [], alternate = true, width, height, objectFit = "cover" }) {
    return (
            ...
            {items.map((item, idx) => {
                ...
                return (
                    <figure key={idx} className={`screenshot-card ${bgClass}`}>
                        <img src={item.src} alt={item.alt || "Screenshot"} style={{ width: resolvedWidth, height: resolvedHeight, objectFit }} />
